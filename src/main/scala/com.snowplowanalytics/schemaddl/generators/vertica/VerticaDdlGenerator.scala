 ...
 */
object VerticaDdlGenerator {
  import vertica.Ddl._
  import vertica.TypeSuggestions._
  import vertica.EncodeSuggestions._
  ...
  private[vertica] val selfDescSchemaColumns = List(
  ...
  private[vertica] val parentageColumns = List(
    Column("root_id", DataTypes.RedshiftChar(36), Set(CompressionEncodings.LzoEncoding), Set(ColumnConstraints.NotNull)),
    Column("root_tstamp", DataTypes.RedshiftTimestamp, Set(CompressionEncodings.DeltaEncoding), Set(ColumnConstraints.NotNull)),
...
//                    .map(c => c.copy(columnName = SU.snakify(c.columnName)))
    ...
    if (rawMode) {
      ...
      val tableAttributes = Set[TableAttribute]( // Snowplow-specific attributes
        ...
        TableAttributes.Sortkey(None, NonEmptyList(
          "\n    schema_vendor",
          "\n    schema_name",
          "\n    schema_format",
          "\n    schema_version",
  ...
  private[generators] def getColumnsDdl(flatDataElems: ListMap[String, Map[String, String]], required: Set[String], varcharSize: Int): Iterable[Column] = {
    ...
    for {
      ...
      val constraints =    // only "NOT NULL" now
        if (checkNullability(properties, required.contains(columnName))) Set[ColumnConstraint](ColumnConstraints.Null)
      ...
      //Column(columnName, dataType, columnAttributes = Set.empty[ColumnAttribute], columnConstraints = constraints)
    }
  }
